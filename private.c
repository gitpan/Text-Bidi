...
XS(_wrap_log2vis) {
  {
    ...
    *_global_p_len = (FriBidiStrIndex)(size1/sizeof(FriBidiChar));
    ...
    if ( _global_wantarray ) {
      Newx(*temp5, ((*_global_p_len) + 1), FriBidiStrIndex);
    }
    if ( _global_wantarray ) {
      Newx(*temp6, ((*_global_p_len) + 1), FriBidiStrIndex);
    }
    if ( _global_wantarray ) {
      Newx(*temp7, ((*_global_p_len) + 1), FriBidiLevel);
    }
    _saved[0] = ST(1);
    ...
    *_global_p_len = (FriBidiStrIndex)(size1/sizeof(FriBidiChar));
    ...
    if (items > 1) {
      if (SvOK(ST(1))) {
        ...
        buf = (AV*)(SvRV(ST(1)));
        size = av_len(buf) + 1;
        Newx(arg4, size, FriBidiStrIndex);
        for(i=0;i < size; i++) {
          SV** sv = av_fetch(buf, i, 0);
          if ( sv )
          arg4[i] = (FriBidiStrIndex)(SvUV(*sv));
          else
          arg4[i] = -1;
        }
      }
